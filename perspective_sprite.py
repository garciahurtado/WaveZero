from displayio import Group
import time
import math

SCREEN_WIDTH = 96
SCREEN_HEIGHT = 64

class PerspectiveSprite():
    """ Simulates a sprite in a 3D space given a 2D plane and a vanishing point
    """
    def __init__(self, sprite_grid, x=0, y=0, z=0, horiz_y=0):
        self.sprite_grid = sprite_grid
        self.bitmap = sprite_grid.bitmap
        self.num_frames = round(self.bitmap.width/sprite_grid.tile_width) - 1
        print(f"Num frames: {self.num_frames}")
        self.horiz_y = horiz_y
        
        # Calculate vanishing point based on the horizon
        vp_x = round(SCREEN_WIDTH / 2)
        vp_y = horiz_y
        self.vanishing = {"x": vp_x, "y": vp_y}
        self.focal_length = 100 # Distance from the camera to the projection plane in pixels
        self.horiz_z = 2000
        
        self.x = x
        self.y = y
        self._z = z
        
    @property
    def z(self):
        return self._z
    
    @z.setter
    def z(self, value):
        self._z = value
        self.update_sprite()


    def update_sprite(self):
        """ Update the real sprites x,y coordinates based on a projection from the fake 3D sprites x,y,z coords
        """
        if self._z == 0:
            self._z = self.horiz_z + 1
        
        # invert the y, since y is at the bottom in our 3D space
        invert_y = SCREEN_HEIGHT - self.y
        
        # Here's where the magic happens. We convert the 3D coordinates to x,y in 2D space
        # @link https://math.stackexchange.com/a/2338025
        self.sprite_grid.x = round(self.x * ( self.focal_length / self._z) + self.vanishing['x'])
        self.sprite_grid.y = round(invert_y * ( self.focal_length / self._z)) + self.bitmap.height
        
        # Resize the sprite (change the frame in the sheet)
        px_range = SCREEN_HEIGHT - self.horiz_y
        if self._z >= self.horiz_z:
            self.sprite_grid[0] = 0
        elif self._z < 0:
            self.sprite_grid[0] = self.num_frames
        else:
            frame_num = round((self._z / self.horiz_z) * (self.num_frames))
            
            # invert it
            frame_num = self.num_frames - frame_num
            self.sprite_grid[0] = frame_num
        
        
        
    
        